...
class ImageProcessor():
    ...
    def saveImage(self):
        '''сохраниь копию файла в подсказке'''
    ...
    def do_flip(self):
        self.image = self.image.transpose(Image.RIGHT_90)
        self.saveImage()
        image_path = os.path.join(workdir, self.save_dir, self.filename)
        self.showImage(image_path)

    def do_flip(self):
        self.image = self.image.transpose(Image.FLIP_LEIP_RIGHT)
        self.saveImage()
        image_path = os.path.join(workdir, self.save_dir, self.filename)
        self.showImage(image_path)
...
def showChosenImage():
    if lw_files.currentRow() >= 0:
        filename = lw_files.connectItem().text()
...
btn_dir.clicked.connect(workimage.do_bw)
